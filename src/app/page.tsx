...
export default function StorefrontPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface font-sans">
      ...
      <nav className="sticky top-0 z-50 bg-white border-b border-outline-variant">
        <div className="max-w-6xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-body-sm font-medium text-on-surface-variant">
            <a
              href="#inicio"
              className="hover:text-primary transition-colors border-b-2 border-primary text-primary pb-0.5"
            >
              Início
            </a>
            <a
      ...
      <section
        id="inicio"
        className="relative h-[480px] md:h-[520px] flex items-center overflow-hidden"
      >
        <div className="absolute inset-0 bg-surface-container-highest">
          ...
          <div
            ...
            style={{
              backgroundImage:
                "repeating-linear-gradient(0deg, transparent, transparent 40px, #006c4f 40px, #006c4f 41px), repeating-linear-gradient(90deg, transparent, transparent 40px, #006c4f 40px, #006c4f 41px)",
            }}
          />
        </div>
        <div className="relative max-w-6xl mx-auto px-4 md:px-8 w-full">
          <div className="max-w-xl">
            <h1 className="text-[32px] md:text-[48px] font-semibold leading-tight tracking-tight text-white">
              Encomende os seus{" "}
              <span className="text-primary-container">blocos</span> com
              precisão industrial.
            </h1>
            <p className="mt-4 text-[16px] text-white/80 leading-relaxed">
              Material de alta resistência para construção civil.
              <br />
      ...
      <section id="catalogo" className="max-w-6xl mx-auto px-4 md:px-8 py-16">
        <div className="flex items-end justify-between mb-8">
          ...
          <span className="text-label-caps text-on-surface-variant">
            Preços actualizados: Hoje
          </span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {storefrontProducts.map((product) => {
            const inCart = cart.find((i) => i.product.type === product.type);
            return (
              <div
                key={product.type}
              ...
              >
                ...
                <div className="p-4 flex flex-col gap-3 flex-1">
                  <div>
                    ...
                    <p className="text-label-caps text-on-surface-variant mt-1 leading-relaxed">
                      {product.description}
                    </p>
                  </div>
                  ...
                  <div className="flex items-end justify-between mt-auto">
                    <div>
                      ...
                      <p className="text-[20px] font-bold text-on-surface">
                        MT {product.pricePerUnit.toLocaleString("pt-MZ")},00
                      </p>
                    </div>
                  ...
                  {inCart ? (
                    <div className="flex items-center justify-between border border-outline-variant rounded-md px-3 h-10">
                      <button
                        onClick={() =>
                          updateQty(product.type, -product.minimumOrder)
                        }
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-body-sm text-on-surface-variant mt-2">
              Processo simplificado em 3 etapas
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: "Seleccione o tipo e a quantidade necessária para o seu projecto.",
              },
              {
                step: "2",
                title: "Localização",
                desc: "Indique o local de descarga em Nampula ou arredores para cálculo de frete.",
              },
              {
                step: "3",
                title: "Pagar",
                desc: "Confirme o pagamento via M-Pesa, E-Mola ou transferência bancária.",
              },
            ].map((s, i) => (
              <div
                key={s.step}
              ...
              >
                {i < 2 && (
                  <div className="hidden md:block absolute top-6 left-[calc(50%+28px)] right-0 h-px bg-outline-variant" />
                )}
                ...
                <h3 className="text-body-sm font-semibold text-on-surface mt-4">
                  {s.title}
                </h3>
                <p className="text-label-caps text-on-surface-variant mt-2 leading-relaxed">
                  {s.desc}
                </p>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <p className="text-body-sm text-on-surface-variant mt-4 leading-relaxed">
              Nossa frota está preparada para entregas em terrenos de difícil
              acesso. Garantimos a integridade de cada bloco do carregamento até
              à descarga.
            </p>
            <div className="mt-6 flex flex-col gap-3">
              {[
                "Nampula Cidade e Arredores",
                "Nacala e Nacala-a-Velha",
                "Ilha de Moçambique e Monapo",
                "Angoche e Distritos Costeiros",
              ].map((zone) => (
      ...
      <footer className="bg-surface-container-low border-t border-outline-variant">
        ...
        <div className="border-t border-outline-variant">
          <div className="max-w-6xl mx-auto px-4 md:px-8 py-4 flex flex-col md:flex-row justify-between items-center gap-2">
            <span className="text-label-caps text-on-surface-variant">
              © 2024 BlokManager Moz. Todos os direitos reservados.
            </span>
            <div className="flex gap-6">
              <a
                href="#"
                className="text-label-caps text-on-surface-variant hover:text-primary transition-colors underline"
              >
                Termos de Uso
              </a>
              <a
          ...
          {cartOpen && (
            <div className="w-80 rounded-md border border-outline-variant bg-white shadow-xl p-4 flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2 max-h-48 overflow-y-auto">
                {cart.map((item) => (
                  <div
                    key={item.product.type}
                    className="flex items-center justify-between"
                  >
                    <div>
                      <p className="text-label-caps font-medium text-on-surface">
                        {item.product.name}
                      </p>
                      <p className="text-label-caps text-on-surface-variant">
                        MT {item.product.pricePerUnit} × {item.quantity}
                      </p>
                    </div>
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => updateQty(item.product.type, -50)}
                        className="w-6 h-6 rounded border border-outline-variant flex items-center justify-center hover:bg-surface-container"
                      >
                        <Minus className="h-3 w-3" />
                      </button>
                      <span className="w-8 text-center text-label-caps font-medium">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() => updateQty(item.product.type, 50)}
              ...
              <div className="border-t border-outline-variant pt-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-body-sm font-bold text-on-surface">
                    MT {totalPrice.toLocaleString("pt-MZ")},00
                  </p>
                </div>
                <Button
